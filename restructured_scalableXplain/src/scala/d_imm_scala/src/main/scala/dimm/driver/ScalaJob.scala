...
//     val maxBins = argMap.getOrElse("--maxBins", "32").toInt
...
//     val (tree, splits) = IMMRunner.runIMM(clusteredInstances, model.clusterCenters, numSplits = maxBins, maxBins = maxBins)
...
//     treeAssigned.take(5).foreach { case (clusterId, features) =>
//       println(s"Cluster $clusterId → Features: ${features.toArray.mkString("[", ", ", "]")}")
//     }
